...
public class HumanBoneController : MonoBehaviour
{
    ...
    Transform[] m_BoneMapping = new Transform[k_NumSkeletonJoints]; //Declaring the BoneMapping array of 90 joints

    //Initializes Skeleton Joints of the Apparel
    public void InitializeSkeletonJoints() 
    {
        ...
        Queue<Transform> nodes = new Queue<Transform>(); // Declaring a queue to traverse through the joints
        nodes.Enqueue(m_SkeletonRoot);
        while (nodes.Count > 0) //Iterates till it traverses all the joints
        {
            Transform next = nodes.Dequeue();
            for (int i = 0; i < next.childCount; ++i)//Adds all the child joints of that Joint to the queue
            {
    ...
    //Validates the joint and initializes the BoneMapping array
